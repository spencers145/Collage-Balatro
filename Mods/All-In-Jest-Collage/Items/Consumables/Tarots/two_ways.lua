local two_ways = {
    ...
    can_use = function(self, card, area, copier)
        if G.hand and (#G.hand.highlighted <= card.ability.max_highlight and #G.hand.highlighted > 0) then
            for i = 1, #G.hand.highlighted do
                if G.hand.highlighted[i].base.id < 6 then
                    return false
                end
            end
            return true
    ...
    use = function(self, card)
        ...
        for i = 1, #G.hand.highlighted do
            ...
            local _card2 = copy_card(hand_card, nil, nil, G.playing_card)

            local new_rank_id_for_clone2 = nil
            if hand_card.base.id % 3 == 1 then
                new_rank_id_for_clone2 = math.max(2, math.min(math.floor(hand_card.base.id / 3), 14))
            else
                new_rank_id_for_clone2 = math.max(2, math.min(math.ceil(hand_card.base.id / 3), 14))
            end

            local new_rank_id_for_clone = math.max(2, math.min(math.floor(hand_card.base.id / 3), 14))
            local new_rank_id_for_original = math.max(2, math.min(math.ceil(hand_card.base.id / 3), 14))

            local new_rank_name_for_clone = rank_id_to_name[new_rank_id_for_clone]
            local new_rank_name_for_clone2 = rank_id_to_name[new_rank_id_for_clone2]
            ...
            assert(SMODS.change_base(_card2, nil, new_rank_name_for_clone2))
            ...
            G.E_MANAGER:add_event(Event({
                func = function()
                    ...
                    playing_card_joker_effects({ _card2 })
                    return true
                end
            }))
        end
