...
Set `prefix_config.key = false` on your object instead.]]):format(obj.key), obj.set)
    ...
    SMODS.Consumable = SMODS.Center:extend {
        ...
        inject = function(self)
            ...
            if self.hidden then
                ...
                self.soul_rate = self.soul_rate or 0.002
    ...
    SMODS.Booster:take_ownership_by_kind('Standard', {
        ...
        create_card = function(self, card, i)
            ...
            local _seal = SMODS.poll_seal({mod = 10, options = {"Red", "Blue", "Purple", "Gold"}})
            return {set = (pseudorandom(pseudoseed('stdset'..G.GAME.round_resets.ante)) > 0.6) and "Enhanced" or "Base", edition = _edition, seal = _seal, area = G.pack_cards, skip_materialize = true, soulable = true}
        end,
        loc_vars = pack_loc_vars,
    })
    ...
    SMODS.Sticker{
        ...
        apply = function(self, card, val)
            card.ability[self.key] = val
            if card and card.ability and card.ability.set and card.ability.set == "Spectral" then
                if card.ability[self.key] then card.ability.perish_tally = 3 end
            elseif card and card.ability and card.ability.set and card.ability.set == "collectable" then
                if card.ability[self.key] then card.ability.perish_tally = 2 end
            else
        ...
        loc_vars = function(self, info_queue, card)
            if card and card.ability and card.ability.set and card.ability.set == "Spectral" then
                return {vars = {card.ability.perishable_rounds or 3, card.ability.perish_tally or 3}}
            elseif card and card.ability and card.ability.set and card.ability.set == "collectable" then
                return {vars = {card.ability.perishable_rounds or 2, card.ability.perish_tally or 2}}
            else
    ...
    SMODS.Enhancement = SMODS.Center:extend {
        ...
        generate_ui = function(self, info_queue, card, desc_nodes, specific_vars, full_UI_table)
            ...
            if specific_vars and specific_vars.bonus_chips and not type(specific_vars.bonus_chips) == "string" then
                local remaining_bonus_chips = to_big(specific_vars.bonus_chips) - (self.config.bonus or 0)
