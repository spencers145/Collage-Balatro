...
function Card.get_chip_bonus(self)
  return get_chip_bonus_ref(self) + (((self:is_suit('bunc_Fleurons') or self:is_suit('bunc_Halberds')) and (self.base.suit == 'bunc_Fleurons' or self.base.suit == "bunc_Halberds")) and 10 or 0)
end
...
function BUNCOMOD.content.process_loc_text()
    ...
    G.P_CENTERS['bunc_consumable_edition_ortalab_anaglyphic'] = {key = 'bunc_consumable_edition_ortalab_anaglyphic', set = 'Other'}
    G.P_CENTERS['bunc_consumable_edition_bunc_fluorescent'] = {key = 'bunc_consumable_edition_bunc_fluorescent', set = 'Other'}
end
    ...
    if G.jokers ~= nil then --Jokers that affect money income
        for _, v in ipairs(SMODS.find_card('j_bunc_fiendish')) do
            if not v.debuff and to_big(mod) > to_big(0) then
                if SMODS.pseudorandom_probability(v, pseudorandom('fiendish'), 1, v.ability.extra.odds, 'fiendish') then
                    mod = to_big(1)
        ...
        if G.playing_cards then
            ...
            for _, suit_data in pairs(SMODS.Suits) do
                ...
                for _, deck_card in ipairs(G.playing_cards) do
                    ...
                    if not deck_card.config.center.no_rank and deck_card.base.suit == suit then
...
create_joker({ -- Head in the Clouds
    ...
    check_for_unlock = function(self, args)
        if args.type == 'win_custom' then
            ...
            for k, v in pairs(G.GAME.hands) do
                if to_big(v.level) > to_big(level) or (to_big(v.level) == to_big(level) and to_big(order) > to_big(v.order)) then
                    level = v.level
                    handname = k
                end
            end
    ...
    custom_in_pool = function()
        ...
        if G.GAME and G.GAME.hands then
            if to_big(G.GAME.hands['High Card'].level) > to_big(1) then condition = true end
        end
        return condition
    end,
    calculate = function(self, card, context)
        if context.level_up_hand and context.level_up_hand ~= self.name then
            if SMODS.pseudorandom_probability(card, pseudorandom('head_in_the_clouds'), 1, card.ability.extra.odds, 'head_in_the_clouds') then
                event({func = function()
...
create_joker({ -- Trigger Finger
    ...
    calculate = function(self, card, context)
        if context.bunc_trigger_finger_highlight_card and (G.STATE == G.STATES.SELECTING_HAND or G.STATE == G.STATES.DRAW_TO_HAND) then
            ...
            if #cards > 0 and SMODS.pseudorandom_probability(card, pseudorandom('trigger_finger'), 1, card.ability.extra.odds, 'trigger_finger') then
                for i = 1, #cards do
...
create_joker({ -- Puzzle Board
    ...
    calculate = function(self, card, context)
        if context.using_consumeable and context.consumeable.ability.set == 'Tarot' then
            if SMODS.pseudorandom_probability(card, pseudorandom('puzzle_board'), 1, card.ability.extra.odds, 'puzzle_board') then
                local cards = {}
...
create_joker({ -- Vandalism
    ...
    custom_vars = function(self, info_queue, card)
        local n,d = SMODS.get_probability_vars(card, 1, card.ability.extra.odds)
		return {
			vars = { n, d, card.ability.extra.xmult }
		}
    end,
    rarity = 'Rare', cost = 6,
    blueprint = true, eternal = true,
    ...
    calculate = function(self, card, context)
        if context.stay_flipped and context.to_area == G.hand then
            ...
            if G.STATE == G.STATES.SELECTING_HAND or G.STATE == G.STATES.DRAW_TO_HAND then
                if SMODS.pseudorandom_probability(card, pseudorandom('vandalism'), 1, card.ability.extra.odds, 'vandalism') then
...
create_joker({ -- Critic
    ...
    calculate = function(self, card, context)
        if context.joker_main then
            ...
            if to_big(hand_chips) * to_big(mult) < (to_big(temp_chips) / to_big(card.ability.extra.fraction)) then return {
                Xmult_mod = card.ability.extra.xmult,
                card = card,
                message = localize {
                    type = 'variable',
...
create_joker({ -- Wino
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play then
            ...
            if context.other_card:is_suit('Diamonds') and context.other_card:is_suit('Hearts') then
                return {
                    dollars = 1,
                    mult = 2,
                    chips = 20,
...
create_joker({ -- Mousetrap
    ...
    custom_vars = function(self, info_queue, card)
        local n,d = SMODS.get_probability_vars(card, 1, card.ability.extra.odds)
		return {
			vars = { card.ability.extra.chips, n, d }
		}
    end,
    rarity = 'Uncommon', cost = 6,
    blueprint = true, eternal = true,
    ...
    calculate = function(self, card, context)
        ...
        if context.joker_main then
            if SMODS.pseudorandom_probability(card, pseudorandom('mousetrap'), 1, card.ability.extra.odds, 'mousetrap') then
            ...
            if context.scoring_hand and not context.other_card then
                ...
                for k, v in ipairs(context.scoring_hand) do
                    if v.config.center == G.P_CENTERS.c_base then
                        if SMODS.pseudorandom_probability(card, pseudorandom('thejoker'), 1, card.ability.extra.odds, 'thejoker') then
                            table.insert(card.ability.extra.trash_list, v)
                        end
                    end
                end
    ...
    calculate = function(self, card, context)
        ...
        local function play_stylophone(other_card)
            ...
            if SMODS.has_no_rank(other_card) then
                event({trigger = 'after', func = function() play_sound(instrument, 1.0, 1.15) return true end})
            else
...
create_joker({ -- Kite Experiment
    ...
    calculate = function(self, card, context)
        if context.rescore_cards then
            ...
            if condition and SMODS.pseudorandom_probability(card, pseudorandom('kite_experiment'), 1, card.ability.extra.odds, 'kite_experiment') then
...
create_joker({ -- Envious
    ...
    vars = {{s_mult = 7}, {suit = 'bunc_Fleurons'}},
    rarity = 'Common', cost = 5,
    blueprint = true, eternal = true,
    unlocked = true,
    effect = 'Suit Mult'
...
create_joker({ -- Proud
    ...
    vars = {{s_mult = 7}, {suit = 'bunc_Halberds'}},
    rarity = 'Common', cost = 5,
    blueprint = true, eternal = true,
    unlocked = true,
    effect = 'Suit Mult'
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and context.other_card:is_suit('bunc_Halberds') then
            ...
            for key, value in pairs(context.scoring_hand) do
                if not value:is_suit('bunc_Halberds') then
                    total = total + 1
                end
            end
            ...
            if total >= 1 then
                return {
                    collage_reduce_blind = card.ability.extra.reduction * total
                }
            end
        end
    end
...
create_joker({ -- Wishalloy
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and context.other_card:is_suit('bunc_Fleurons') then
            if SMODS.pseudorandom_probability(card, pseudorandom('wishalloy'), 1, card.ability.extra.odds, 'wishalloy') then
...
create_joker({ -- Unobtanium
    ...
    unlocked = true,
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play and context.other_card:is_suit('bunc_Halberds') and context.other_card.base.suit == 'bunc_Halberds' then
            return {
                collage_reduce_blind = card.ability.extra.reduction * PB_UTIL.get_unique_suits(context.scoring_hand)
            }
        end
    end
})
...
create_joker({ -- Starfruit
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and not context.other_card and not context.blueprint then
            if SMODS.pseudorandom_probability(card, pseudorandom('starfruit'), 1, card.ability.extra.odds, 'starfruit') then
...
create_joker({ -- ROYGBIV
    ...
    calculate = function(self, card, context)
        if context.before and context.poker_hands ~= nil and next(context.poker_hands['bunc_Spectrum']) and context.scoring_hand and not context.blueprint then
            if SMODS.pseudorandom_probability(card, pseudorandom('roygbiv'), 1, card.ability.extra.odds, 'roygbiv') then
                if context.scoring_hand then
    ...
    loc_vars = function(self, info_queue)
        ...
        if G.hand then
            for _, value in pairs(G.hand.cards) do
                if value:is_face() or value:get_id() == 6 or value:get_id() == 9 or value:get_id() == 14 then
                    reward = reward + self.config.bonus
                end
            end
        end
        return {vars = {self.config.bonus, reward}}
    end,
...
SMODS.Consumable{ -- Cleanse
    ...
    use = function(self, card)
        ...
        for i = 1, #G.hand.highlighted do
            event({trigger = 'after', delay = 0.1, func = function()
                --unlock_card(G.P_CENTERS.b_ortalab_neon)
...
function link_cards(cards, source, ignore_groups)
    ...
    if cards ~= nil and #cards > 0 then
        if math.random() > 0.5 then
            play_sound('collage_band1', 0.9 + 0.2 * math.random(), 2)
        else
            play_sound('collage_band2', 0.9 + 0.2 * math.random(), 2)
        end
    end
end
...
discovered = false, -- The Tine
    ...
    recalc_debuff = function(self, card, from_blind)
        if not G.GAME.blind.disabled and card.area ~= G.jokers and not card.config.center.no_rank then
...
SMODS.Tag{ -- Breaking
    ...
    in_pool = function()
        local chance = 0.07
        if G.GAME.current_round and G.GAME.current_round.used_cine and #G.GAME.current_round.used_cine > 0 then chance = 0.4 end
        if pseudorandom('breaking'..G.SEED) < chance then
            return true
        else
            return false
        end
...
SMODS.Tag{ -- Arcade
    ...
    in_pool = function()
        local chance = 0.1
        if G.GAME.current_round and G.GAME.current_round.used_cine and #G.GAME.current_round.used_cine > 0 then chance = 0.2 end
        if pseudorandom('arcade'..G.SEED) < chance then
            return true
        else
            return false
        end
...
SMODS.Tag{ -- Xmult
    ...
    in_pool = function() 
        return G.GAME.current_round and G.GAME.current_round.used_cine and #G.GAME.current_round.used_cine > 0
    end
}
...
SMODS.Edition{
    ...
    extra_cost = 1,
    loc_vars = function (self, info_queue, card)
        return {key = card and card.config and card.config.center.set == 'Joker' and 'e_bunc_fluorescent_joker'}
    end,
    get_weight = function(self)
        return G.GAME.edition_rate * self.weight
    end,
    calculate = function(self, card, context)
        if (context.cardarea == G.jokers and context.end_of_round) or
        (context.main_scoring and context.cardarea == G.play) or
        (context.cardarea == G.consumeables and context.end_of_round) then
            return {
                dollars = 2
            }
        end
...
SMODS.Voucher{ -- Lamination
    ...
    redeem = function (self, voucher)
        unlock_card(G.P_CENTERS.b_ortalab_experimental)
    end
}
...
SMODS.Voucher{ -- Chainsaw
    ...
    redeem = function(self)
        ...
        unlock_card(G.P_CENTERS.b_vis_gardening)
    end,
...
SMODS.Voucher{ -- Disguise
    ...
    atlas = 'bunco_vouchers',
    redeem = function (self, voucher)
        unlock_card(G.P_CENTERS.b_paperback_passionate)
    end
}
            ...
            local t = {n=G.UIT.ROOT, config = {align = 'tm', r = 0.15, colour = G.C.CLEAR, padding = 0.15}, nodes={
                ...
                {n=G.UIT.R, config={align = "tm"}, nodes={
                    ...
                    {n=G.UIT.C,config={align = "tm", padding = 0.05, minw = 2.4}, nodes={
                        ...
                        (nil and not G.GAME.rerolled_pack and
                        ...
                        nil and {n=G.UIT.R,config = {minh = 0.07}, nodes={}},
                        {n=G.UIT.R, config = {align = nil and 'cm' or 'tm', padding = nil and 0.07 or 0.2, minh = nil and 0.7 or 1.2, minw = 1.8, r = 0.15, colour = G.C.GREY, one_press = true, button = 'skip_booster', hover = true, shadow = true, func = 'can_skip_booster'}, nodes = {
...
SMODS.Enhancement({ -- Cracker
    ...
    loc_vars = function (self, info_queue, card)
        local n,d = SMODS.get_probability_vars(card, 1, 2, 'bunc_cracker_break')
        return {vars = {n, d}}
    end,
        ...
        for _, cracker_card in ipairs(context.full_hand) do
            if cracker_card.marked_cracker and cracker_card.config.center == G.P_CENTERS.m_bunc_cracker and not cracker_card.debuff then
                if not cracker_card.destroyed then
                    if not SMODS.pseudorandom_probability(cracker_card, pseudoseed("bunc_cracker_break"), 1, 2, 'bunc_cracker_break') then
                        forced_message("Nope!", cracker_card)
                        event({
                            trigger = 'before',
                            delay = 0.7*2*1.051,
                            func = function()
