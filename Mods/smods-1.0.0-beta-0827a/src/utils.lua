...
function SMODS.create_mod_badges(obj, badges)
    if not SMODS.config.no_mod_badges and obj and obj.mod and obj.mod.display_name and not obj.no_mod_badges then
        ...
        for i, mod in ipairs(mods) do
            ...
            for key, value in pairs(COLLAGE_MODIFIED_TABLE_MINOR) do
                if value == obj.key and obj.mod == mod then
                    mod_name = mod_name .. "*"
                    break
                end
            end

            -- same until ive made my decision about whether i like the 2-system
            for key, value in pairs(COLLAGE_MODIFIED_TABLE_MAJOR) do
                if value == obj.key and obj.mod == mod then
                    mod_name = mod_name .. "*"
                    break
                end
            ...
            for key, value in pairs(COLLAGE_MODIFIED_TABLE) do
                if value == obj.key and obj.mod == mod then
                    mod_name = mod_name .. "*"
                    break
                end
            ...
            local max_text_width = 2.5 - 2*0.05 - 4*0.03*size - 2*0.03
            ...
            badges[#badges + 1] = {n=G.UIT.R, config={align = "cm"}, nodes={
                {n=G.UIT.R, config={align = "cm", colour = mod.badge_colour or G.C.GREEN, r = 0.1, minw = 2.5, minh = 0.36, emboss = 0.05, padding = 0.03*size}, nodes={
...
function SMODS.calculate_context(context, return_table, no_resolve)
    ...
    --if not G.GAME.joker_context_calculated then
    --    G.GAME.joker_context_calculated = true
    --    print("joker context calculation resulted in")
    --    print(G.GAME.context_count)
    --end
    context.main_eval = nil
    --local start = os.clock()
    if G.GAME.context_count == 1 then print("playing cards calculation begins with 1") end
...
function SMODS.get_next_vouchers(vouchers)
    ...
    for i=#vouchers+1, math.min(SMODS.size_of_pool(_pool), G.GAME.starting_params.vouchers_in_shop + (G.GAME.modifiers.extra_vouchers or 0)) do
        local decided = false
        if not vouchers.spawn['v_hieroglyph'] and G.GAME.starting_params.archeology_deck then
            for key, value in pairs(_pool) do
                if value == 'v_hieroglyph' then
                    vouchers[#vouchers+1] = 'v_hieroglyph'
                    vouchers.spawn['v_hieroglyph'] = true
                    decided = true
                    break
                end
            end
...
function SMODS.localize_box(lines, args)
    ...
    for _, part in ipairs(lines) do
        ...
        for _, subpart in ipairs(part.strings) do
            assembled_string = assembled_string..(type(subpart) == 'string' and subpart or (args.vars and format_ui_value(args.vars[tonumber(subpart[1])])) or 'ERROR')
        end
...
function Card:should_hide_front()
  return self.ability.effect == 'Stone Card' or self.config.center.replace_base_card or self.config.bleached
end
