...
--table.insert(SMODS.calculation_keys, "aij_balance_percent_full")
if SMODS.other_calculation_keys then
    table.insert(SMODS.other_calculation_keys, "aij_balance_percent")
    --table.insert(SMODS.other_calculation_keys, "aij_balance_percent_full")
end
...
SMODS.calculate_individual_effect = function(effect, scored_card, key, amount, from_edition)
    ...
    --[[if key == "aij_balance_percent_full" then
        if amount > 1 then
            amount = 1
        end
        if effect.card and effect.card ~= scored_card then juice_card(effect.card) end
